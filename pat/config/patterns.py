...
PATTERN_VERSION = "1.4.0"  # bumped for DOB/phone/address/secret refinements
...
PATTERN_DEFINITIONS: Sequence[PatternDefinition] = (
    ...
    PatternDefinition(
        ...
        category="FINANCIAL",
        regex=r"\b(?:account|acct|a\/c)\s*(?:number|no\.?)?\s*(?:is\s*)?[:\-]?\s*(\d{8})\b",
        confidence=0.92,
        group=1,
    ...
    PatternDefinition(
        id="loyalty_id_prefixed",
        pii_type="CUSTOMER_ID",
        category="FINANCIAL",
        regex=r"\b(LOY-\d{4}-\d{4})\b",
        confidence=0.8,
        group=1,
        priority=52,
    ),
    ...
    PatternDefinition(
        ...
        pii_type="CREDENTIAL",
        category="AUTHENTICATION",
        regex=r"\b(?:password|passcode|pwd|secret|jwt[_-]?secret|api[_-]?key|token)\s*[:=]\s*[\"']?([^\s\"']{6,})[\"']?",
        confidence=0.9,
        group=1,
        validator="password",
        priority=8,
    ...
    PatternDefinition(
        id="api_key_stripe_like",
        pii_type="API_KEY",
        category="AUTHENTICATION",
        regex=r"\b(sk_(?:live|test)_[A-Za-z0-9]{10,})\b",
        confidence=0.92,
        group=1,
        priority=12,
    ),
    PatternDefinition(
        id="api_key_env_style",
        pii_type="API_KEY",
        category="AUTHENTICATION",
        regex=r"\b[A-Z0-9_]*?(?:API_KEY|ACCESS_KEY|SECRET|PASSWORD|TOKEN)\s*=\s*\"?([A-Za-z0-9_\-]{8,})\"?",
        confidence=0.86,
        group=1,
        priority=18,
    ),
    ...
    PatternDefinition(
        id="credential_env_value",
        pii_type="CREDENTIAL",
        category="AUTHENTICATION",
        regex=r"\b[A-Z0-9_]{3,32}\s*=\s*[\"']([^\"']{6,})[\"']",
        confidence=0.93,
        group=1,
        priority=6,
    ),
    ...
    PatternDefinition(
        ...
        pii_type="PHONE",
        category="CONTACT",
        regex=r"(?<!\w)(\+?\d[\d\-\s()]{8,}\d)\b",
        confidence=0.82,
        group=1,
        validator="phone_len",
        keywords=("phone", "mobile", "call", "tel", "telephone", "number"),
        priority=25,
    ),
    PatternDefinition(
    ...
    PatternDefinition(
        id="postcode_keyvalue",
        pii_type="POSTCODE",
        category="CONTACT",
        regex=r"\b(?:shipping_postcode|billing_postcode|postcode)\s*=\s*\"?([A-Z]\d{2}\s?[A-Z0-9]{4})\"?",
        confidence=0.9,
        group=1,
        priority=19,
    ),
    PatternDefinition(
        ...
        pii_type="ADDRESS",
        category="CONTACT",
        regex=r"\b(Apartment|Apt\.?|Flat|Wohnung)\s+\d+\b",
        confidence=0.8,
        group=0,
        priority=42,
    ),
    PatternDefinition(
        id="address_postcode_city",
        pii_type="ADDRESS",
        category="CONTACT",
        regex=r"\b(\d{4,5}\s+[A-Z][A-Za-z]+)\b",
        confidence=0.8,
        group=1,
        priority=45,
    ),
    ...
    PatternDefinition(
        ...
        group=1,
        validator="date",
        keywords=("dob", "birth", "date of birth", "dob=", "dob:"),
        priority=48,
    ),
    PatternDefinition(
        id="date_dob_keyword",
    ...
    PatternDefinition(
        ...
        pii_type="DATE",
        category="CONTACT",
        regex=r"\b(?:dob|date_of_birth|date-of-birth)\s*[:=]\s*\"?(\d{4}-\d{2}-\d{2})\"?",
        confidence=0.85,
        group=1,
        validator="date",
        priority=44,
    ...
    PatternDefinition(
        ...
        regex=r"\b([A-CEGHJ-PR-TW-ZQ]{2}\s?\d{2}\s?\d{2}\s?\d{2}\s?[A-D])\b",
        confidence=0.9,
        group=1,
        validator="nino",
        priority=5,
    ...
    PatternDefinition(
        id="passport_alphanum_keyworded",
        pii_type="PASSPORT_NUMBER",
        category="GOVERNMENT_ID",
        regex=r"\b([A-Z]{1,2}\d{7,9})\b",
        confidence=0.84,
        group=1,
        validator="passport",
        keywords=("passport",),
    ...
    PatternDefinition(
        ...
        pii_type="GOV_ID",
        category="GOVERNMENT_ID",
        regex=r"\b(?:tax\s*id|steuer[- ]?id)[:\-]?\s*([0-9 ]{11,})\b",
        confidence=0.9,
        group=1,
        priority=16,
    ),
    # HEALTH
    PatternDefinition(
        id="health_condition_terms",
        pii_type="MEDICAL_INFO",
        category="HEALTH",
        regex=r"\b(chronic\s+asthma|asthma|depression|anxiety|panic\s+attack|mental\s+health|chronic\s+migraine|migraine)\b",
        confidence=0.88,
        group=1,
        priority=35,
    ),
